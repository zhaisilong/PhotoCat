...
def main(
    ...
    for ec_level in ec_levels:
        ...
        # 交叉验证然后需要把 'reactants' 列和 'products' 列分别输出
        def write_cv_splits(df, ec_level, output_dir, n_splits=5):
            _df = df.copy()
            fold_df = kfold(_df)
            for fold in range(n_splits):
                df_fold_test = fold_df[fold_df[f'fold{fold}']]
                df_fold_train = fold_df[~fold_df[f'fold{fold}']]
                df_fold_test_tgt = df_fold_test['products']
                df_fold_test_src = df_fold_test['reactants']
                df_fold_train_tgt = df_fold_train['products']
                df_fold_train_src = df_fold_train['reactants']
                fold_dir = output_dir / str(fold)
                fold_dir.mkdir(exist_ok=True)
                df_fold_test_tgt.to_csv(fold_dir / "tgt-test.txt", index=False, header=False)
                df_fold_test_src.to_csv(fold_dir / "src-test.txt", index=False, header=False)
                df_fold_test_tgt.to_csv(fold_dir / "tgt-valid.txt", index=False, header=False)
                df_fold_test_src.to_csv(fold_dir / "src-valid.txt", index=False, header=False)
                df_fold_train_tgt.to_csv(fold_dir / "tgt-train.txt", index=False, header=False)
                df_fold_train_src.to_csv(fold_dir / "src-train.txt", index=False, header=False)

        # 进去之前 shuffle 一遍
        from sklearn.utils import shuffle
        df_tmp = shuffle(df_tmp, random_state=42)
        df_tmp.reset_index(inplace=True, drop=True)
        write_cv_splits(df_tmp, ec_level, parent_dir, n_splits=5)
